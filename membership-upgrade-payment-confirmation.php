    ...
    if (is_array($response) && count($response) > 0 && $response['ResponseMessage']) {
        ...
        if ($response['ResponseCode'] == 0 && $response['ResponseMessage'] == 'Transaction Successful' && $response['MerchantRefNo'] > 0 && $response['Amount'] > 0) {
            if ($objMembers->verify_payment($response['MerchantRefNo'], $response['Amount'])) {
                $affected_rows = $objMembers->approve_upgrade_membership_plan($response['MerchantRefNo'], $response);
                ...
                $objBase->Redirect('membership-upgrade-payment-confirmation.php?TransactionID=' . $response['TransactionID']);
            } else {
                $objBase->Redirect('membership-upgrade-payment-canceled.php?msg=payment-amount-miss-match');
            }
        } else {
            $objBase->Redirect('membership-upgrade-payment-canceled.php');
        }
    }
    ?>
